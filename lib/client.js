...
function DiscogsClient(userAgent, oauth, options){
	...
	// Set configuration for this instance to the defaults and merge in any passed options
	this.config = {};
...
DiscogsClient.prototype.getRequestToken = function(consumerKey, consumerSecret, callbackUrl, callback){
	...
	this._rawRequest({url: this.config.oauthRequestUrl+'?oauth_callback='+oa.percentEncode(callbackUrl)}, function(err, data){
		if(!err){
			if(data){ data = queryString.parse(data); }
			oauth.token = data.oauth_token;
			oauth.tokenSecret = data.oauth_token_secret;
			oauth.authorizeUrl = this.config.oauthAuthorizeUrl+'?oauth_token='+data.oauth_token;
		}
...
DiscogsClient.prototype.getAccessToken = function(verifier, callback){
	...
	oauth.status = 'access';
	this._rawRequest({url: this.config.oauthAccessUrl+'?oauth_verifier='+oa.percentEncode(verifier)}, function(err, data){
		if(!err){
...
DiscogsClient.prototype.about = function(callback){
	...
	this.get('', function(err, data){
		if(data){ data.disconnect = { version: pkg.version, userAgent: (self.userAgent||this.config.customHeaders['User-Agent']) }; }
		callback(err, data);
	}); 
};
...
DiscogsClient.prototype._rawRequest = function(options, callback){
	...
	headers = merge(headers, this.config.customHeaders);
