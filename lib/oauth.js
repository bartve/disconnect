...
class DiscogsOAuth {
    ...
    constructor(authData) {
        ...
        authStore.set(this, {method: 'oauth', level: 0});
    ...
    setConfig(customConfig) {
        if (customConfig && (typeof customConfig === 'object')) {
            Util.merge(configStore.get(this), customConfig);
        }
        return this;
    }
    ...
    getAccessToken(verifier, callback) {
        let auth = authStore.get(this);
        new Client(auth).get({url: configStore.get(this).accessTokenUrl + '?oauth_verifier=' + OAuth.prototype.percentEncode(verifier), queue: false, json: false}, (err, data) => {
            if (!err && data) {
                data = queryString.parse(data);
                auth.token = data.oauth_token;
                auth.tokenSecret = data.oauth_token_secret;
     ...
     */
    export() {
        return Util.merge({}, authStore.get(this));
    }
