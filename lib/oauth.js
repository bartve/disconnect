...
class DiscogsOAuth {
    ...
    getRequestToken(consumerKey, consumerSecret, callbackUrl, callback) {
        ...
        new Client(auth).get({
            url: config.requestTokenUrl + '?oauth_callback=' + OAuth.prototype.percentEncode(callbackUrl),
            queue: false,
            json: false
        }, (err, data) => {
            if (!err && data) {
    ...
    getAccessToken(verifier, callback) {
        let auth = authMap.get(this);
        new Client(auth).get({
            url: configMap.get(this).accessTokenUrl + '?oauth_verifier=' + OAuth.prototype.percentEncode(verifier),
            queue: false,
            json: false
        }, (err, data) => {
            if (!err && data) {
