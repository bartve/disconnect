 ...
 * @type {WeakMap}
 */
const stackStore = new WeakMap();

/**
 * Store the queue instance call info privately in a WeakMap
 * @type {WeakMap}
 */
const callInfoStore = new WeakMap();
...
class Queue {
    ...
    constructor(customConfig) {
        ...
        stackStore.set(this, []);
        callInfoStore.set(this, {firstCall: 0, callCount: 0});
    ...
    add(callback) {
        let config = configStore.get(this),
            callInfo = callInfoStore.get(this);
        
        // No current stack? Check the call count
        if (stackStore.get(this).length === 0) {
            var now = Date.now();
     ...
     */
    clear() {
        for (let item of stackStore.get(this)) {
