...
module.exports = function(client){
	...
	/**
	 * Get all lists of given username
	 * @param {(number|string)} user - Username
	 * @param {object} [params] - Optional pagination params
	 * @param {function} [callback] - The callback
	 * @return {DiscogsClient|Promise}
	 */
	
	list.getLists = function(username, params, callback){
		var path = '/users/'+util.escape(username)+'/lists';
		if((arguments.length === 2) && (typeof params === 'function')){
			callback = params;
		}else{
			path = util.addParams(path, params);
